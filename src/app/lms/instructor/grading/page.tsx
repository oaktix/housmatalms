...
export default function InstructorGrading() {
  ...
  const handleAiRestructure = async () => {
    ...
    setAiLoading(true);
    try {
      const res = await fetch("/api/ai/restructure-feedback", {
        method: "POST",
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "Failed to restructure feedback");
      }
            ...
            {selectedSub ? (
              <div className="premium-card rounded-2xl bg-bg-card border-border-main p-6 sm:p-8 space-y-6 shadow-md animate-fade-in">
                ...
                <form onSubmit={handleGradeSubmit} className="space-y-4 pt-4 border-t border-border-main/50">
                  ...
                  <div className="form-group flex flex-col gap-1">
                    <label htmlFor="feedback" className="text-[10px] font-bold text-text-muted uppercase tracking-wider flex items-center justify-between">
                      <span>Evaluation &amp; Feedback Notes</span>
                      <button
                        type="button"
                        onClick={handleAiRestructure}
                        disabled={aiLoading}
                        className="btn border border-primary/30 hover:bg-primary/5 text-primary px-3 py-1 rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {aiLoading ? (
                          <>
